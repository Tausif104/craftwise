...
/**
 * Tracking IDs for the public site, read once per request by PageLayout.
 *
 * `consentRequired` is the switch in SEO & Tracking. It defaults to true and a
 * failed query also yields true, so a database problem can never be the reason
 * tracking loads without consent.
 */
export async function getPublicTrackingSetting() {
  const setting = await prisma.trackingSetting
    .findUnique({ where: { environment: "production" } })
    .catch(() => null);

  return {
    measurementId: setting?.ga4MeasurementId || process.env.NEXT_PUBLIC_GA_ID || null,
    gtmContainerId: setting?.gtmContainerId || null,
    consentRequired: setting?.consentRequired ?? true,
  };
}
