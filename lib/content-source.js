...
export async function getTestimonials(pageKey, locale = "de") {
  try {
    const placements = await prisma.testimonialPlacement.findMany({
      ...
      // The placement carries the editor's order; the item order and creation
      // date only break ties so the sequence never shuffles between requests.
      orderBy: [
        { sortOrder: "asc" },
        { testimonial: { sortOrder: "asc" } },
        { testimonial: { createdAt: "asc" } },
      ],
...
export async function getFaqs(pageKey, locale = "de") {
  try {
    const placements = await prisma.faqPlacement.findMany({
      ...
      orderBy: [
        { sortOrder: "asc" },
        { faqItem: { sortOrder: "asc" } },
        { faqItem: { createdAt: "asc" } },
      ],
    ...
    return placements.map(({ faqItem }) => ({
      ...
      category: faqItem.category
        ? {
            ...
            sortOrder: faqItem.category.sortOrder,
          }
        : null,
    }));
  } catch (error) {
...
export async function getFaqsByCategory(pageKey, locale = "de") {
  ...
  for (const item of items) {
    ...
    if (!grouped.has(key)) {
      grouped.set(key, {
        ...
        // Uncategorised questions sort last instead of wherever they happened
        // to appear in the item list.
        sortOrder: item.category ? item.category.sortOrder ?? 0 : Number.MAX_SAFE_INTEGER,
        items: [],
      });
    }
  ...
  // Section order on the FAQ page follows the category order set in the admin.
