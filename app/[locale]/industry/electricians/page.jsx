...
import { electricianCommonChallengesData } from '@/data/commonChallangesData'
import { electriciansSolvesData } from '@/data/industrySolveData'
import { electriciansKeyBenefitsData } from '@/data/keyBenifitsData'
import { electricianstestimonials } from '@/data/testimonials'
import { electricianWorkflowData } from '@/data/workFlowStepsData'
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: t('electriciansTitle'),
    alternates: getLocalizedAlternates({ locale, href: '/industry/electricians' }),
  }
}

const page = async ({ params }) => {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'Electricians' })
  ...
  const kbData = {
    ...electriciansKeyBenefitsData,
    headingParts: [
      { text: t('kbPart1'), className: 'text-secondary' },
      { text: t('kbPart2'), className: 'text-primary' },
    ],
    items: electriciansKeyBenefitsData.items.map((item, i) => ({
      ...item,
  ...
  const wsData = {
    ...electricianWorkflowData,
    heading: t('wsHeading'),
    steps: electricianWorkflowData.steps.map((step, i) => ({
      ...step,
  ...
  const ccData = {
    ...electricianCommonChallengesData,
    title: {
  ...
  return (
    <PageLayout ctaData={ctaData} pageKey="/industry/electricians" locale={locale}>
      <Hero
        pageName={nav('electricians')}
        slug={nav('industry')}
        slugLink={`/${locale}/industry`}
        title={[{ text: t('heroTitle'), primary: false }]}
        button={{ text: t('heroCta'), link: '/registration' }}
        description={t('heroDescription')}
        image="/images/hero/i_electrician.svg"
      />
      <CommonChallanges data={ccData} />
      <CarftwiseSolves data={csData} />
      <WorkflowSteps data={wsData} />
      <KeyBenefits data={kbData} />
      <TestimonialSection pageKey="/industry/electricians" locale={locale}         heading={t('testimonialHeading')}
        subtext={t('testimonialSubtext')}
        data={{
          items: electricianstestimonials.items.map((item, i) => ({
            ...item,
      ...
      />
      <FAQ pageKey="/industry/electricians" locale={locale}         heading={t('faqHeading')}
        faqItems={[
